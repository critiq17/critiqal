...
describe('adminService', () => {
	...
	it('login posts credentials with the skip flag', async () => {
	...
	it('verifyTwoFactor posts challenge and code with skip flag', async () => {
		...
		expect(post).toHaveBeenCalledWith(
			'/api/admin/auth/2fa',
			{ challengeToken: 'tok', code: '123456' },
			SKIP
		);
	});
	...
	it('searchUsers defaults page=0 size=20', async () => {
		get.mockResolvedValue({ content: [], page: 0, size: 20, total: 0, hasNext: false });
		await adminService.searchUsers('');
		expect(get).toHaveBeenCalledWith('/api/admin/users/search?q=&page=0&size=20');
	});
	...
	it('revokeBadge deletes the badge path', async () => {
