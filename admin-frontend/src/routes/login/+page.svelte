<script lang="ts">
	...
	async function submitCredentials(event: SubmitEvent): Promise<void> {
		...
		try {
			...
			// 404 is the masked denial for invalid creds or non-admin; generic message.
			error =
				e instanceof ApiError && e.isNotFound ? 'Invalid credentials' : 'Something went wrong';
		} finally {
			loading = false;
		}
	}
	...
	async function submitCode(event: SubmitEvent): Promise<void> {
		...
		try {
			...
			// 401 = wrong TOTP code; anything else = expired/invalid challenge.
...
<svelte:head><title>critiqal admin — login</title></svelte:head>
