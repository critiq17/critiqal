<script lang="ts">
	...
	type ResultState = 'idle' | 'loading' | 'loaded' | 'error';
	...
	// Track which query produced the current results so tab switches don't re-fetch
	...
	async function fetchResults(q: string, tab: Tab): Promise<void> {
		...
		try {
			if (tab === 'posts') {
				posts = q.trim() ? await postService.search(q.trim()) : await postService.getFeed();
			} else {
	...
	function handleUserKeydown(e: KeyboardEvent, username: string): void {
	...
	<main class="col-center" aria-label="Explore">
		...
		<div class="content-area" role="tabpanel" aria-label={activeTab === 'posts' ? 'Posts results' : 'People results'}>
			...
			{#if resultState === 'loading'}
				<!-- Skeleton state — no spinners, only shimmer skeletons -->
				{#if activeTab === 'posts'}
