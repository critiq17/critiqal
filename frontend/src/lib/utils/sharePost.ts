...
function postUrl(post: Post): string {
	const origin = typeof window !== 'undefined' ? window.location.origin : 'https://critiqal.app';
	// Deep link to the author with the post id — self-contained, no new route.
	return `${origin}/${post.author.username}?p=${post.id}`;
}
...
function shareText(post: Post): string {
	...
	return snippet ? `${who} on Critiqal: “${snippet}${post.content.length > 80 ? '…' : ''}”` : `${who} on Critiqal`;
}
