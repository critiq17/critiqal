<script lang="ts">
	...
	function handleReplyKey(e: KeyboardEvent, commentId: number): void {
		if (e.key === 'Enter' && (e.ctrlKey || e.metaKey)) {
			comments.submitReply(commentId);
		}
		if (e.key === 'Escape') {
			comments.closeReplyComposer(commentId);
		}
	}
</script>
	...
	{:else if comments.comments.length > 0}
		<ul class="comments-list">
			{#each comments.comments as comment (comment.id)}
				{@const rs = comments.getReplyState(comment.id)}
	...
	{#if authStore.isAuthenticated}
		<div class="comment-compose">
			<div class="comment-compose-inner">
				<textarea
					class="comment-input"
					bind:value={comments.newComment}
				...
				<button
					...
					disabled={!comments.newComment.trim() || comments.submitting}
					aria-label="Post comment"
				>
					{comments.submitting ? '…' : 'Post'}
				</button>
			</div>
		</div>
	{/if}
