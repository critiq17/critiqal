<script lang="ts">
  ...
  import BadgeIcon from '$lib/components/badges/BadgeIcon.svelte';
  import { badgeMeta, tierStyle } from '$lib/components/badges/badgeMeta';
  ...
  async function toggle(badge: AdminBadge): Promise<void> {
    ...
    try {
      ...
      onfeedback(owned ? `Could not revoke ${badge.name}` : `Could not grant ${badge.name}`, 'error');
    } finally {
      pending = null;
    }
  }
...
<aside class="panel" aria-label={`Manage badges for ${current.username}`}>
  <header class="panel-head">
    <div class="who">
      <Avatar src={current.avatarUrl} name={current.name ?? current.username} size={40} />
      <div class="who-text">
  ...
  <ul class="badge-grid">
    {#each badges as badge (badge.code)}
      ...
      <li>
        ...
        >
          <span class="badge-icon" aria-hidden="true"><BadgeIcon glyph={meta.glyph} size={20} /></span>
