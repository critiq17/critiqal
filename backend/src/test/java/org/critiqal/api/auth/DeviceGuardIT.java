...
class DeviceGuardIT {
    ...
    void register_newDevice_returns201() {
        given()
                ...
                .body("{\"username\":\"device_first_user\",\"password\":\"pass123\"}")
    ...
    void register_sameDevice_secondAccount_returns409() {
        ...
        given()
                ...
                .body("{\"username\":\"device_conflict_1\",\"password\":\"pass123\"}")
                .when().post("/api/auth/register")
                .then().statusCode(201);
        ...
        given()
                .contentType(JSON)
                .header("X-Device-Id", deviceId)
                .body("{\"username\":\"device_conflict_2\",\"password\":\"pass123\"}")
                .when().post("/api/auth/register")
                .then().statusCode(409)
    ...
    void register_sameDevice_afterLogout_stillReturns409() {
        ...
        var sid = given()
                .contentType(JSON)
                .header("X-Device-Id", deviceId)
                .body("{\"username\":\"device_conflict_3\",\"password\":\"pass123\"}")
                .when().post("/api/auth/register")
                .then().statusCode(201)
        ...
        given()
                .contentType(JSON)
                .header("X-Device-Id", deviceId)
                .body("{\"username\":\"device_conflict_4\",\"password\":\"pass123\"}")
                .when().post("/api/auth/register")
                .then().statusCode(409)
                .body("error", equalTo("device_already_used"));
    }
    ...
    void register_noDeviceId_returns201_allowedFallback() {
        given()
                .contentType(JSON)
                .body("{\"username\":\"device_no_header_user\",\"password\":\"pass123\"}")
                .when().post("/api/auth/register")
                .then().statusCode(201);
    }
    ...
    void getSessions_returnsListWithCurrentMarker() {
        var sid = given()
                ...
                .body("{\"username\":\"sessions_list_user\",\"password\":\"pass123\"}")
                .when().post("/api/auth/register")
                .then().statusCode(201)
                .extract().cookie("session");
